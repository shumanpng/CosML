# This code is from https://github.com/wyharveychen/CloserLookFewShot/blob/master/methods/meta_template.py and https://github.com/hytseng0509/CrossDomainFewShot/blob/master/methods/meta_template.py
# import backbone
import torch
...
class MetaTemplate(nn.Module):
    def __init__(self, model_func, n_way, n_support, model_info = None, pretrained_feature_extractor = None, change_way = True, tf_path = None):
        super(MetaTemplate, self).__init__()
        self.n_way      = n_way
        self.n_support  = n_support
        self.n_query    = -1 #(change depends on input)
        if pretrained_feature_extractor is not None:
            self.intermediate_feature = pretrained_feature_extractor
        self.feature    = model_func#()
        self.model_info = model_info # (feature_extractor_model, task_network_model)
        if self.feature is not None:
            self.feat_dim   = self.feature.final_feat_dim
        else:
            if self.model_info == ('resnet10', 'linear'):
                self.feat_dim = 512
            else:
                self.feat_dim = self.intermediate_feature.final_feat_dim
    ...
    def test_loop(self, test_loader, record = None, verbose = True):
        correct = 0
        count = 0
        loss = 0.
        acc_all = []
